...
export class Authentication1697987841784 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`CREATE TABLE \`EmailToken\` (\`id\` varchar(64) NOT NULL, \`token\` text NOT NULL, \`tokenType\` enum ('forgotpassword', 'registration') NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`roleId\` varchar(64) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
		);
		await queryRunner.query(
			`ALTER TABLE \`Branch\` ADD CONSTRAINT \`FK_60a206d10a93dfa31319f529086\` FOREIGN KEY (\`userId\`) REFERENCES \`User\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`,
		);
		await queryRunner.query(
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`ALTER TABLE \`Branch\` DROP FOREIGN KEY \`FK_60a206d10a93dfa31319f529086\``,
		);
